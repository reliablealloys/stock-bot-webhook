"""
Add scrap search functionality to main.py

This patch adds a special handler for "scrap" queries
"""

# Add this function after search_inventory_flexible()
...
# Modify the message handler to check for "scrap" keyword
# In handle_message(), add this before the existing search:

    # Check if query is for scrap
    if 'scrap' in text.lower():
        scrap_results = search_scrap()
        if scrap_results:
            response = format_scrap_response(scrap_results)
